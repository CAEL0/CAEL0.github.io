...
만약 a가 x개, b가 y개, c가 z개 있다면 (WLOG, x >= y >= z) x <= y + z를 만족해야 shuffled anagram을 만들 수 있다.
...
$$ \gamma = \lim_{n \rightarrow \infin} (\sum_{k = 1}^n \frac{1}{k} - \ln n) = \int_{1}^{\infin} (\frac{1}{\lfloor x \rfloor} - \frac{1}{x})dx = 0.5772156649... $$
